...
def run_pass1_5_coarse_partition(
    ...
    sql = f"""
        COPY (
            ...
            SELECT base.* EXCLUDE (_coarse_lon_lo, _coarse_lon_hi, _coarse_lat_lo, _coarse_lat_hi),
                lon_t.coarse_lon AS coarse_lon, lat_t.coarse_lat AS coarse_lat
            FROM base,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pass1-only",
        action="store_true",
        help="Stop after Pass 1 (intermediates only). Use tile_v12_pass1_5.py and tile_v12_pass2.py to finish.",
    )
...
def main() -> None:
    ...
    if args.pass1_only:
        print(f"\nPass 1 done in {pass1_elapsed/60:.1f} min. Stopping (--pass1-only).", flush=True)
        return
